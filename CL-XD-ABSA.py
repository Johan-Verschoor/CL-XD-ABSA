# Method for obtaining cross-domain feature representations.
#
# This method is based on two key components:
# 1. The LCR-Rot-hop++ model from Trușcă et al. (2020), which serves as the foundational architecture for
#    aspect-based sentiment analysis.
# 2. The domain and class discriminator introduced by Knoester et al. (2022), which extends the model
#    to facilitate domain adversarial training, enabling better generalization across different domains.
#
# In addition to these components, I  (Johan Verschoor) have further expanded the model by integrating a contrastive
# loss mechanism. Both the sentiment components and the domain components can be expanded with a contrastive loss
# function.
#
# The model is implemented as part of the CL-XD-ABSA framework, which explores cross-domain aspect-based
# sentiment analysis with advanced representation learning techniques.
#
...
def main(train_path_source, train_path_target, test_path, learning_rate_dis=0.03, learning_rate_f=0.03, keep_prob=0.3,
         momentum_dis=0.80, momentum_f=0.85, l2_dis=0.001, l2_f=0.001, balance_lambda=0.80, tau_d=0.1, tau_c=0.1, lambda_dcl=0.2, lambda_ccl=0.0):
    """
    ...
    with tf.device('/gpu:1'):
        ...
        # Compute the contrastive loss for both domains, or set to 0
        contrastive_domain_loss = contrastive_loss(outputs_fin_combined, d_combined, tau_d)
        #contrastive_domain_loss = 0
        ...
        # Compute the contrastive class loss for the source domain, or set to 0
        # #contrastive_class_loss = contrastive_loss(outputs_fin_source, y_src, tau_c)
        contrastive_class_loss=0
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(FLAGS.n_iter):
            ...
            for train, numtrain, train_count_t in get_batch_data(tr_x_src, tr_x_tar, te_x, tr_sen_len_src,
                ...
                #Ocasionally run t-SNE, to save storage
                o=random.random()
                if o < 0.01:  # ~1% chance
                    out_combined_val, d_combined_val = sess.run(
                        [outputs_fin_combined, d_combined],
                        feed_dict=train
                    )
